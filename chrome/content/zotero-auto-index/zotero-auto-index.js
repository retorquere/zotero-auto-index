Zotero.AutoIndex = {
  diacriticsRemoval: [
    {
      ...
      replacement: 'A', // alpha
      regex: /[\u0410\u0391\u0041\u24B6\uFF21\u00C0\u00C1\u00C2\u1EA6\u1EA4\u1EAA\u1EA8\u00C3\u0100\u0102\u1EB0\u1EAE\u1EB4\u1EB2\u0226\u01E0\u00C4\u01DE\u1EA2\u00C5\u01FA\u01CD\u0200\u0202\u1EA0\u1EAC\u1EB6\u1E00\u0104\u023A\u2C6F]/g
    }, {
      replacement: 'AA',
      regex: /[\uA732]/g
    }, {
      ...
      replacement: 'I',
      regex: /[\u026A\x49\u24BE\uFF29\xCC\xCD\xCE\u0128\u012A\u012C\u0130\xCF\u1E2E\u1EC8\u01CF\u0208\u020A\u1ECA\u012E\u1E2C\u0197]/g
    }, {
      replacement: 'J',
      regex: /[\x4A\u24BF\uFF2A\u0134\u0248\u0237]/g
    }, {
      ...
      regex: /[\u0435\u1F10\u1F73\u1f10\u1F15\u03B5\u03AD\u0065\u24D4\uFF45\u00E8\u00E9\u00EA\u1EC1\u1EBF\u1EC5\u1EC3\u1EBD\u0113\u1E15\u1E17\u0115\u0117\u00EB\u1EBB\u011B\u0205\u0207\u1EB9\u1EC7\u0229\u1E1D\u0119\u1E19\u1E1B\u0247\u01DD]/g
    }, {
      replacement: 'f',
      regex: /[\u03D5\u03C6\u0066\u24D5\uFF46\u1E1F\u0192]/g
    }, {
      ...
      regex: /[\u043E\u1F41\u1F45\u1F44\u1F79\u03BF\u03CC\u00BA\x6F\u24DE\uFF4F\xF2\xF3\xF4\u1ED3\u1ED1\u1ED7\u1ED5\xF5\u1E4D\u022D\u1E4F\u014D\u1E51\u1E53\u014F\u022F\u0231\xF6\u022B\u1ECF\u0151\u01D2\u020D\u020F\u01A1\u1EDD\u1EDB\u1EE1\u1EDF\u1EE3\u1ECD\u1ED9\u01EB\u01ED\xF8\u01FF\uA74B\uA74D\u0275\u0254\u1D11]/g
    }, {
      replacement: 'oe',
      regex: /[\u0153]/g
    }, {
      ...
      replacement: 't',
      regex: /[\u0442\u03B8\u03D1\u03C4\x74\u24E3\uFF54\u1E6B\u1E97\u0165\u1E6D\u021B\u0163\u1E71\u1E6F\u0167\u01AD\u0288\u2C66\uA787]/g
    }, {
      replacement: 'th',
      regex: /[\u00FE]/g
    }, {
      ...
      regex: /[\u03CD\u1F50\u1F55\u1F7B\u1FE6\u03C5\u03C7\x75\u24E4\uFF55\xF9\xFA\xFB\u0169\u1E79\u016B\u1E7B\u016D\xFC\u01DC\u01D8\u01D6\u01DA\u1EE7\u016F\u0171\u01D4\u0215\u0217\u01B0\u1EEB\u1EE9\u1EEF\u1EED\u1EF1\u1EE5\u1E73\u0173\u1E77\u1E75\u0289]/g
    }, {
      replacement: 'v',
      regex: /[\x76\u24E5\uFF56\u1E7D\u1E7F\u028B\uA75F\u028C]/g
    }, {
  ...
  unicode: {
    ...
    // Title case.
    Lt: /\u01C5\u01C8\u01CB\u01F2\u1F88\u1F89\u1F8A\u1F8B\u1F8C\u1F8D\u1F8E\u1F8F\u1F98\u1F99\u1F9A\u1F9B\u1F9C\u1F9D\u1F9E\u1F9F\u1FA8\u1FA9\u1FAA\u1FAB\u1FAC\u1FAD\u1FAE\u1FAF\u1FBC\u1FCC/,
  ...
  expandWordList: function(words) {
    ...
    for (word of words) {
      ...
      for (let i = 0; i < strlen; i++) {
        ...
        if (ca == undefined) {
          if (c.match(this.asciiRegex)) {
            ca = c;
          } else {
            for (pattern of this.diacriticsRemoval) {
              if (c.match(pattern.regex)) {
                ca = pattern.replacement;
                break;
              }
            }
          }
          ...
          if (!ca) {
            this.log('failed to fold unicode for "' + c + '" (0x' + c.charCodeAt(0).toString(16) + ')');
            ca = null;
          }
          this.diacriticsRemovalMap[c] = ca;
        }
        if (ca) {
          ascii += ca;
        } else {
      ...
      if (ascii) { expanded[ascii.toLowerCase()] = true; }
  ...
  log: function(msg, e) {
    if (!this.prefs.getBoolPref('debug')) { return; }
    msg = '[' + this.DBNAME + '] ' + msg;
    if (e) {
  ...
  init: function () {
    ...
    // monkey-patch convoluted indexWords
    Zotero.Fulltext.indexWords = (function (self, original) {
      return function (itemID, words) {
        ...
        Zotero.DB.query('INSERT INTO fulltextItemWords (wordID, itemID) SELECT DISTINCT wordID, ? FROM fulltextWords ftw JOIN autoindex.fulltextwords aftw ON aftw.word = ftw.word', [itemID]);
        Zotero.DB.query("REPLACE INTO fulltextItems (itemID, version) VALUES (?,?)", [itemID, 1]); // Zotero.Fulltext.FULLTEXT_VERSION = 1 but private to the namespace
        
        Zotero.DB.commitTransaction();
        self.log('Indexing done for ' + itemID);
        return true;
      }
    })(this, Zotero.Fulltext.indexWords);

    // monkey-patch ridiculously slow semanticSplitter
    Zotero.Fulltext.semanticSplitter = (function (self, original) {
    ...
    })(this, Zotero.Fulltext.semanticSplitter);

    // monkey-patch indexString to use this.semanticSplitter
    Zotero.Fulltext.indexString =  (function (self, original) {
      return function (text, charset, itemID) {
        self.log('indexString started');
        ...
        self.log('indexString finished');
      }
    })(this, Zotero.Fulltext.indexString);
    ...
    Zotero.Fulltext.indexFile = (function (self, original) {
      return function (file, mimeType, charset, itemID, maxLength, isCacheFile) {
        self.log('indexFile(' + file + ',' + mimeType + ',' + charset + ',' + itemID + ',' + maxLength + ',' + isCacheFile + ')');
        try {
    ...
    if (this.prefs.getBoolPref('zotfile.enabled')) {
      Zotero.ZotFile.pdfAnnotations.createNote = (function (self, original) {
        return function (annotations, item, att, method) {
          ...
          try {
            ...
            if (oldnotes.size > 0) {
              oldnote = Zotero.DB.valueQuery('SELECT noteID from autoindex.annotations WHERE itemID = ? AND noteID in (' + ['?' for (note of oldnotes)].join(',') + ')', [att.id] + oldnotes);
            }
          } catch (err) {
          ...
          try {
            ...
            self.log('createNote:' + att.id + ',' + att.attachmentModificationTime + ',' + newnote);
            Zotero.DB.query('REPLACE INTO autoindex.annotations (itemID, checksum, noteID) VALUES (?,?,?)', [att.id, att.attachmentModificationTime, newnote]);
          } catch (err) {
            self.log('createNote', err);
          }
      ...
      })(this, Zotero.ZotFile.pdfAnnotations.createNote);
    }
  ...
  zotFileUpdateAnnotations: function() {
    if (!this.prefs.getBoolPref('zotfile.enabled')) { return; }
    ...
    let items = [item.id for (item of (Zotero.Items.getAll() || [])) if (item.isAttachment() && checksum[item.id] != item.attachmentModificationTime)];
    items = items.splice(0, this.prefs.getIntPref('zotfile.batch'));
    this.log('processing ' + JSON.stringify(items));

    try {
      Zotero.ZotFile.pdfAnnotations.getAnnotations(items);
    } catch (err) {
      this.log('Zotero.ZotFile.pdfAnnotations.getAnnotations', err);
    }
    this.log('zotFileUpdateAnnotations: updated');
  },
  ...
  reindexRequest: function(request) { this.reindexItem(Zotero.Sync.Storage.getItemFromRequestName(request.name)); },

  reindex: function(itemID) {
    Zotero.DB.query('DELETE FROM fulltextItemWords WHERE itemID = ?', [itemID]);
    Zotero.DB.query('DELETE FROM fulltextItems WHERE itemID = ?', [itemID]);
    this.log('Marked for re-indexing: ' + itemID);
  },
